        ...
        <el-card>
          ...
          <div class="table-wrapper">
            <el-table v-loading="loading" element-loading-text="加载数据中" :data="tableData" border>
                ...
                <template slot-scope="props">
                  <el-form label-position="left" class="table-expand">
                    ...
                    <el-form-item label="公司介绍">
                      ...
                      <viewer :images="[props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[0]]">
                        <img class="img" :src="props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[0]" alt="">
                      </viewer>
                    </el-form-item>
                    <el-form-item label="产品介绍">
                      <div>{{props.row.goodsPicList[0].intro2}}</div>
                      <viewer :images="[props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[1]]">
                        <img class="img" :src="props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[1]" alt="">
                      </viewer>
                    </el-form-item>
                    <el-form-item label="食用方法介绍">
                      <div>{{props.row.goodsPicList[0].intro3}}</div>
                      <viewer :images="[props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[2]]">
                        <img class="img" :src="props.row.goodsPicList[0].picPath + '/' + props.row.goodsPicList[0].goodsPicName.split(',')[2]" alt="">
                      </viewer>
                    </el-form-item>
                  </el-form>
                </template>
              ...
              <el-table-column label="审核状态" align="center" v-if="type == '3'">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.status == '1'" size="mini" type="info">已通过</el-button>
                  ...
                  <el-button v-if="scope.row.status == '4'" size="mini" type="info">已上架</el-button>
                  <el-button v-if="scope.row.status == '5'" size="mini" type="info">已下架</el-button>
                  <el-button v-if="scope.row.status == '6'" size="mini" type="info">已驳回</el-button>
                </template>
              </el-table-column>
              <el-table-column label="操作" align="center" v-if="type == '3'">
                <template slot-scope="scope">
                  <el-button
                    v-if="scope.row.status == '4'"
                    size="mini"
                    type="warning"
                    @click="goodsDown(scope.row)"
                  >下架</el-button>
                  <el-button
        ...
        if (valid) {
          if (this.dialogType == "1") {
            ...
            if (this.addPage == "2") {
              ...
              let formData = new FormData();
              formData.append("id", this.id);
              let distribution = this.ruleForm1.distribution[
                this.ruleForm1.distribution.length - 1
              ];
              if (this.falseType == "1") {
                formData.append("name", this.ruleForm1.name);
                formData.append("specs", this.ruleForm1.specs);
